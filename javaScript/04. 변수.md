...
1. 애플리케이션 : 데이터를 입력<p style="color: teal">input</p>받아 처리<p style="color: teal">process</p> → 결과를 출력<p style="color: teal">output</p> (IPO)
...
1. 값을 저장하고 참조하는 매커니즘
   ...
   ▸ 값 — <p style="color: mediumslateblue">(컴파일러 or 인터프리터)</p> →  저장된 메모리 공간의 주소
...
- 변수 이름 : 메모리 공간에 저장된 값을 식별할 수 있는 고유한 이름,
  ...
  = 식별자 <p style="color: teal">identifier</p>

- 변수 값 : 변수에 저장된 값, 바뀔 수 있는 값 (재할당)

- 할당 = 대입 = 저장 = assignment : 변수에 값을 저장하는 것

- 참조 <p style="color: teal">reference</p> : 변수에 저장된 값을 읽어들이는 것 참조를 요청하다 = 변수에 저장된 결과를 읽어들여주세요.

- 평가 <p style="color: teal">evaluation</p> : 식을 해석해서 값을 생성하거나 참조하는 것

- 파싱 <p style="color: teal">parsing</p> : 표현식의 의미를 해석

  어떤 페이지에서 내가 원하는 데이터를 특정 패턴이나 순서로 추출해 가공하는 것

  ⇒ 파서가 구문트리로 재구성하는 구문 분석 과정

- 워드 <p style="color: teal">keyword</p>
...
1. 값 저장을 위해 **메모리 공간을 확보**한다. <p style="color: teal">allocate</p>
...
4. 확보된 메모리 공간은 해제<p style="color: teal">release</p>되기 전까지 메모리 공간을 사용할 수 없도록 보호된다. (안전하게 사용 가능)
...
1. **소스코드의 평가 (선언문)**
   ...
   ② 변수, 함수 등의 선언문만 먼저 실행 <p style="color: tomato">(호이스팅)</p>
