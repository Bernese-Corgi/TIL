...
위 예제의 경우 자바스크립트 엔진은 표현식 `x + ''`을 평가하기 위해 x 변수의 숫자값을 바탕으로 새로운 문자열 값 ‘10’을 생성하고 이것으로 표현식 `'10' + ''`를 평가한다. 
...
## [#](https://poiemaweb.com/fastcampus/type-casting#21-문자열-타입으로-변환)2.1. 문자열 타입으로 변환
...
연산자 표현식의 피연산자(피연산자도 표현식이다)만이 암묵적 타입 변환의 대상이 되는 것은 아니다. 앞서 언급했듯이 자바스크립트 엔진은 표현식을 평가할 때 코드 문맥에 부합하도록 암묵적 타입 변환을 실행한다. 예를 들어, ES6에서 도입된 템플릿 리터럴의 표현식 삽입([“6.3.2. 표현식 삽입”](https://poiemaweb.com/fastcampus/type#32-표현식-삽입) 참고)은 표현식의 평가 결과를 문자열 타입으로 암묵적 타입 변환한다.
...
## [#](https://poiemaweb.com/fastcampus/type-casting#22-숫자-타입으로-변환)2.2. 숫자 타입으로 변환
...
## [#](https://poiemaweb.com/fastcampus/type-casting#23-불리언-타입으로-변환)2.3. 불리언 타입으로 변환
...
- 함수
  ...
  함수란 어떤 작업을 수행하기 위해 필요한 문들의 집합을 정의한 코드 블록이다. 함수는 이름과 매개변수를 가지며, 필요할 때 호출해 코드 블록에 담긴 문들을 일괄적으로 실행할 수 있다. 함수에 대해서는 [“12. 함수”](https://poiemaweb.com/fastcampus/function)에서 자세히 살펴보자.

# [#](https://poiemaweb.com/fastcampus/type-casting#3-명시적-타입-변환)3. 명시적 타입 변환
...
## [#](https://poiemaweb.com/fastcampus/type-casting#31-문자열-타입으로-변환)3.1. 문자열 타입으로 변환
...
## [#](https://poiemaweb.com/fastcampus/type-casting#32-숫자-타입으로-변환)3.2. 숫자 타입으로 변환
...
## [#](https://poiemaweb.com/fastcampus/type-casting#33-불리언-타입으로-변환)3.3. 불리언 타입으로 변환
...
# [#](https://poiemaweb.com/fastcampus/type-casting#4-단축-평가)4. 단축 평가

## [#](https://poiemaweb.com/fastcampus/type-casting#41-논리-연산자를-사용한-단축-평가)4.1. 논리 연산자를 사용한 단축 평가

[“7.5. 논리 연산자”](https://poiemaweb.com/fastcampus/operator#5-논리-연산자)에서 설명하지 않고 뒤로 나중으로 미룬 내용이 있다. 그것은 바로 “논리합(`||`) 연산자와 논리곱(`&&`) 연산자 표현식의 평가 결과는 불리언 값이 아닐 수도 있다. 논리합(`||`), 논리곱(`&&`) 연산자 표현식은 언제나 2개의 피연산자 중 어느 한쪽으로 평가된다.”라는 것이다.
...
논리곱(`&&`) 연산자는 두 개의 피연산자가 모두 true로 평가될 때 true를 반환한다. 논리곱 연산자는 좌항에서 우항으로 평가가 진행된다.([“7.13. 연산자의 결합 순서”](https://poiemaweb.com/fastcampus/operator#13-연산자의-결합-순서) 참고) 참고)
...
## [#](https://poiemaweb.com/fastcampus/type-casting#42-옵셔널-체이닝-연산자)4.2. 옵셔널 체이닝 연산자
...
프로퍼티 참조란 변수를 통해 변수값을 참조하듯이 객체의 프로퍼티에 접근해 프로퍼티 값을 참조하는 것을 말한다. 이에 대해서는 [“10.5. 프로퍼티 접근”](https://poiemaweb.com/fastcampus/object-literal#5-프로퍼티-접근)에서 자세히 살펴볼 것이다. 아직 객체에 대해 살펴보지 않았으므로 지금은 간단히 훑어보고 다음 장인 “10. 객체 리터럴”을 살펴본 다음 다시 한번 읽어보도록 하자.
...
옵셔널 체이닝 연산자 ?.는 객체를 가리키기를 기대하는 변수가 null 또는 undefined이 아닌지 확인하고 프로퍼티를 참조할 때 유용하다. 옵셔널 체이닝 연산자 ?.가 도입되기 이전에는 논리 연산자 &&를 사용한 단축 평가를 통해 변수가 null 또는 undefined인지 확인했다.
...
논리 연산자 &&는 좌항 피연산자가 false로 평가되는 Falsy 값(false, undefined, null, 0, -0, NaN, ‘‘)이면 좌항 피연산자를 그대로 반환한다. 좌항 피연산자가 Falsy 값인 0이나 ‘‘인 경우도 마찬가지다. 하지만 0이나 ‘‘은 객체로 평가될 때도 있다. (이에 대해서는 [“21.3. 원시값과 래퍼 객체”](https://poiemaweb.com/fastcampus/built-in-object#3-원시-값과-래퍼-객체)에서 자세히 살펴보자.)
...
## [#](https://poiemaweb.com/fastcampus/type-casting#43-null-병합-연산자)4.3. null 병합 연산자
